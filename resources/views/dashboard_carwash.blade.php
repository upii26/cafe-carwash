...
<div id="appWrapper" class="flex flex-col min-h-screen overflow-y-auto h-screen">
    ...
    <div class="main-layout flex flex-1 overflow-y-auto">
        <div class="flex-1 p-4 md:p-6 pb-20 lg:pb-6">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 mb-5">

                {{-- Pendapatan Hari Ini --}}
                <div class="stat-card bg-[#D4AF37]/15 backdrop-blur-xl border border-[#D4AF37]/30
                ...
                <div class="stat-card bg-[#D4AF37]/15 backdrop-blur-xl border border-[#D4AF37]/30
                    ...
                    <div class="text-xl md:text-2xl font-bold text-[#000]">{{ $totalHariIni }}</div>
                    <div class="text-xs text-[#000] mt-0.5">Total Kendaraan</div>
                    <div class="mt-3 h-1.5 bg-white rounded-full overflow-hidden">
                        <div class="progress-fill" style="width:{{ min(100, ($totalHariIni / 40) * 100) }}%; background:#3b82f6"></div>
                    </div>
                ...
                <div class="stat-card bg-[#D4AF37]/15 backdrop-blur-xl border border-[#D4AF37]/30
                    ...
                    <div class="flex items-start justify-between mb-3">
                        <div class="stat-icon w-10 h-10 rounded-xl bg-green-50 flex items-center justify-center text-xl">🧴</div>
                        <span class="text-[10px] font-semibold text-green-600 bg-green-50 px-2 py-0.5 rounded-full">Hari ini</span>
                    </div>
                    ...
                    <div class="mt-3 h-1.5 bg-white rounded-full overflow-hidden">
                        <div class="progress-fill" style="width:{{ min(100, ($tipeLayanan->sum('total') / 50) * 100) }}%; background:#22c55e"></div>
                    </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 md:gap-4 mb-5">
                ...
                {{-- Bar Chart --}}
                <div class="lg:col-span-2 bg-[#D4AF37]/15 backdrop-blur-xl border border-[#D4AF37]/30
                ...
                {{-- Donut: Tipe Layanan --}}
                <div class="bg-[#D4AF37]/15 backdrop-blur-xl border border-[#D4AF37]/30
                    ...
                    <div class="flex items-center justify-center mb-4">
                        <div class="relative w-28 h-28">
                            <svg viewBox="0 0 36 36" class="w-full h-full donut">
                                <circle cx="18" cy="18" r="14" fill="none" stroke="#e9f0ee" stroke-width="4"/>
                        ...
                        <table class="w-full text-xs">
                            <thead>
                                <tr class="bg-black-50">
                                    ...
                                    <th class="text-left px-3 py-2.5 text-black-400 font-semibold hidden sm:table-cell">Pelanggan</th>
                                    <th class="text-left px-3 py-2.5 text-black-400 font-semibold hidden sm:table-cell">Layanan</th>
                            ...
                            <tbody>
                                @forelse($antrianTerkini as $row)
                                    <tr class="order-row border-t border-black-50">
                                        ...
                                        <td class="px-3 py-3 text-black-500 hidden sm:table-cell">{{ $row->name_customer }}</td>
                                        <td class="px-3 py-3 text-black-500 hidden sm:table-cell truncate max-w-[120px]">{{ $row->layanan }}</td>
                                        <td class="px-3 py-3 font-bold text-black-800">Rp {{ number_format($row->total, 0, ',', '.') }}</td>
                                        <td class="px-3 py-3">
                ...
                <div class="bg-white rounded-2xl p-4 md:p-5 fade-up delay-6">
                        ...
                        @forelse($layananTerlaris as $idx => $item)
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="text-xs font-semibold text-black-800 truncate">{{ $item->name }}</div>
                                    <div class="progress-bar mt-1">
                                        <div class="progress-fill" style="width:{{ $pctBar }}%; background:{{ $col }}"></div>
                                    </div>
                                </div>
                                <span class="text-xs font-bold text-black-700 flex-shrink-0">{{ $item->total_terjual }}x</span>
                            </div>
                        @empty
                    ...
                    <div class="mt-4 pt-4 border-t border-black-50 grid grid-cols-2 gap-3">
                        <div class="bg-[#e6faf6] rounded-xl p-3 text-center">
                            <div class="text-sm font-bold text-[#0BAB8C]">Rp {{ number_format($pendapatanHariIni, 0, ',', '.') }}</div>
