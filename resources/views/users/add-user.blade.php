                    ...
                    <div class="grid grid-cols-1 gap-4 max-w-lg">
                        ...
                        <div class="bg-white rounded-2xl p-5 shadow-sm border border-black/[.07]">
                            <div class="flex items-center gap-2 mb-4">
                                <span
                                    class="w-6 h-6 rounded-lg flex items-center justify-center text-xs font-bold text-white flex-shrink-0"
                        ...
                        <div class="bg-white rounded-2xl p-5 shadow-sm border border-black/[.07]">
                            ...
                            <div class="flex flex-col gap-3">
                                {{-- Owner --}}
                                <label
                                    class="flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all role-option"
                                    id="roleOwner"
                                    style="{{ old('role') === 'owner'
                                    ...
                                    <div class="w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0"
                                        style="background:rgba(212,175,55,.15);color:#8B6B1F">👑</div>
                                    <div class="flex-1">
                                    ...
                                    <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 role-check-owner"
                                        style="{{ old('role') === 'owner' ? 'border-color:#D4AF37;background:#D4AF37' : 'border-color:#cbd5e1' }}">
                                        @if (old('role') === 'owner')
                                ...
                                <label
                                    class="flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all role-option"
                                    id="roleKasir"
                                    style="{{ old('role', 'kasir') === 'kasir' ? 'border-color:#D4AF37;background:rgba(212,175,55,.15)' : 'border-color:#e2e8f0;background:#f8fafb' }}"
                                    ...
                                    <div class="w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0"
                                        style="background:rgba(212,175,55,.15);color:#8B6B1F">🧑‍💼</div>
                                    <div class="flex-1">
                                        <div class="text-sm font-bold" style="color:#1e293b"">Kasir</div>
                                    ...
                                    <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 role-check-kasir"
                                        style="{{ old('role', 'kasir') === 'kasir' ? 'border-color:#D4AF37;background:#D4AF37' : 'border-color:#cbd5e1' }}">
        ...
        function selectRole(role) {
            ...
            roles.forEach(function(r) {
                ...
                if (r === role) {
                    ...
                    card.style.background = 'rgba(212,175,55,.12)';
                    check.style.borderColor = '#D4AF37';
                    check.style.background = '#D4AF37';
                    check.innerHTML =
                        '<svg class="w-3 h-3" style="color:white" fill="none" stroke="white" stroke-width="3" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>';
                } else {
                    card.style.borderColor = '#e2e8f0';
                    card.style.background = '#f8fafb';
                    check.style.borderColor = 'rgba(212,175,55,.35)';
